...
Check Lead Exists
    ...
    Input Text    ${SEARCH_BAR_XPATH}    ${lead_name}
    Click Element    ${SEARCH_ICON_XPATH}
    Sleep    1s
    ${count}=    Get Element Count    //a[text()='${lead_name}']
    IF    ${count} > 0
        RETURN    ${TRUE}
    ELSE
        RETURN    ${FALSE}
...
Create Lead
    ...
    Log To Console    <<<Checking if Lead already exists>>>
    ${lead_exists}=    Check Lead Exists    ${FIRSTNAME_FIELD_TEXT}

    IF    ${lead_exists}
        Log To Console    <<<Lead "${FIRSTNAME_FIELD_TEXT}" already exists, skipping creation>>>
    ELSE
        Log To Console    <<<Lead does not exist, proceeding to create>>>
        ...
        Wait Until Element Is Visible    ${SELECT_GENDER_IDENTITY_XPATH}        5s
        Click Element    ${SELECT_GENDER_IDENTITY_XPATH}
        Click Element    ${LEAD_SOURCE_XPATH}
        Wait Until Element Is Visible    ${SELECT_LEAD_SOURCE_XPATH}        5s
        ...
        Wait Until Element Is Visible    ${SELECT_INDUSTRY_XPATH}       5s
        ...
        Wait Until Element Is Visible     ${SELECT_LEAD_STATUS_XPATH}       5s
        Click Element    ${SELECT_LEAD_STATUS_XPATH}
        Click Element    ${RATING_XPATH}
        Wait Until Element Is Visible    ${SELECT_RATING_XPATH}     5s
        ...
        Wait Until Element Is Visible    ${PREVIOUS_LAST_ACTIVITY_DATE_XPATH}       5s
        ...
        Wait Until Element Is Visible    ${SELECT_LEAD_OWNER_XPATH}     5s
